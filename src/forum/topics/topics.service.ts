...
export class TopicsService {
  constructor(
    ...
    @InjectModel(Topic.name) private readonly topicSchema: Model<TopicDocument>,
  ...
  async updateLastPostAt(topicId: string): Promise<void> {
    await this.topicSchema
      .updateOne({ _id: topicId }, { lastPostAt: new Date() })
      .exec();
  }
  ...
  async findOneById(id: string): Promise<TopicDocument> {
    const topic = await this.topicSchema
      .findById(id)
      .populate({
        path: 'userId',
        select: 'username nickname firstName lastName bio role avatar',
  ...
  async updateOneById(
    ...
    if (updateTopicDto.title && updateTopicDto.title !== topic.title)
      topic.slug = updateTopicDto.slug = await this.createUniqueSlug(
        updateTopicDto.title,
      );

    const updatedTopic = await this.topicSchema
      .findOneAndUpdate({ _id: topic.id }, updateTopicDto, { new: true })
      .exec();
