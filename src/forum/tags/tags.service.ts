...
export class TagsService {
  constructor(
    @InjectModel(Tag.name) private readonly tagSchema: Model<TagDocument>,
  ...
  ): Promise<IPaginationResponse<TagDocument>> {
  ...
  async findAllByUserIdForLibraryMyTagsPaginated(
    ...
    const [tags, total] = await Promise.all([
      this.tagSchema
        ...
        .populate({
          path: 'userId',
          select: 'username role nickname firstName lastName email',
        })
        .sort({ createdAt: -1 })
        .exec(),
      this.tagSchema.countDocuments({ userId }).exec(),
