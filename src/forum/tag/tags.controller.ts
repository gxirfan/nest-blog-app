...
export class TagsController {
  ...
  async createTag(
    ...
    return TagMapper.toSingleResponseDto(
      await this.tagsService.createTag(Number(req.user.id), createTagDto),
    );
  }
  ...
  async findAllPaginated(
    ...
    return { data: TagMapper.toResponseDtoList(data), meta };
  }
  ...
  async findAll(): Promise<TagResponseDto[]> {
    return TagMapper.toResponseDtoList(await this.tagsService.findAll());
  }
  ...
  async findOneBySlug(
    ...
    return TagMapper.toSingleResponseDto(
      await this.tagsService.findOneBySlug(Number(req.user?.id), slug),
    );
  }
  ...
  async updateOneById(
    ...
    return TagMapper.toSingleResponseDto(
      await this.tagsService.updateOneById(
        Number(id),
        Number(req.user.id),
        updateTagDto,
      ),
