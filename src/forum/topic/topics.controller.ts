...
export class TopicsController {
  ...
  async createTopic(
    ...
    return TopicMapper.toSingleResponseDto(
      await this.topicsService.createTopic(Number(req.user.id), createTopicDto),
    );
  }
  ...
  async findAll(): Promise<TopicResponseDto[]> {
    return TopicMapper.toResponseDtoList(await this.topicsService.findAll());
  }
  ...
  ): Promise<IPaginationResponse<TopicResponseDto>> {
    const { data, meta } = await this.topicsService.findAllPaginated(query);
    return { data: TopicMapper.toResponseDtoList(data), meta };
  }
  ...
  async findAllByTagIdPaginated(
    ...
    const { data, meta } = await this.topicsService.findAllByTagIdPaginated(
      tagId,
      query,
    );
    return { data: TopicMapper.toResponseDtoList(data), meta };
  ...
  async findAllByUserIdForLibraryMyTopicsPaginated(
    ...
    const { data, meta } =
      await this.topicsService.findAllByUserIdForLibraryMyTopicsPaginated(
        Number(req.user.id),
        query,
      );
    return { data: TopicMapper.toResponseDtoList(data), meta };
  ...
  @Get(':tagId')
  @ResponseMessage('Topics fetched successfully.')
  async findOneByTagId(
    @Param('tagId') tagId: number,
  ): Promise<TopicResponseDto> {
    return TopicMapper.toSingleResponseDto(
      await this.topicsService.findOneByTagId(tagId),
    );
  }
  ...
  async updateOneById(
    ...
    return TopicMapper.toSingleResponseDto(
      await this.topicsService.updateOneById(
        id,
        Number(req.user.id),
        updateTopicDto,
      ),
